...
function [AATS, SDATS, ASI] = TsUpperVsiEwmaXbarGamSteady(n, hL, hS, lam, K1, K2, delta, a_shape, b_scale)

mu0 = a_shape * b_scale ; % mean for gamma
sigma0 = sqrt(a_shape * b_scale ^ 2) ; % standard deviation for gamma

UWL = mu0 + K1 * sqrt(lam/(n*(2 - lam))) * sigma0 ; % Upper warning limit 
UCL = mu0 + K2 * sqrt(lam/(n*(2 - lam))) * sigma0 ; % Upper control limit 
CL = mu0 ; % centre limit 
...
Q0 = (mu0 - (1 - lam) * hr') / lam ; 
Q(:, 1) = gamcdf(Q0 - delta * sigma0, n * a_shape, b_scale/n) ; 
